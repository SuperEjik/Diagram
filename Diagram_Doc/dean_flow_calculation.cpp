    ...
    ui(new Ui::dean_flow_calculation)
{
    ...
    param = "([0](,)[0-9]{10})||([1-9]{10}(,)[0-9]{10})";

    ui->lineEdit_p->setValidator(new QRegExpValidator(QRegExp("(" + param + "||" + "-(" + param + ")" + ")"), this));
    ui->lineEdit_D->setValidator(new QRegExpValidator(QRegExp("(" + param + "||" + "-(" + param + ")" + ")"), this));
    ui->lineEdit_v->setValidator(new QRegExpValidator(QRegExp("(" + param + "||" + "-(" + param + ")" + ")"), this));
    ui->lineEdit_nu->setValidator(new QRegExpValidator(QRegExp("(" + param + "||" + "-(" + param + ")" + ")"), this));
    ui->lineEdit_Rc->setValidator(new QRegExpValidator(QRegExp("(" + param + "||" + "-(" + param + ")" + ")"), this));
}
...
void dean_flow_calculation::on_flow_De_calculation_clicked()
{
    ...
    Calculation c;
    ui->text_De->insertPlainText(QString::number(c.definition_Dn(p, D, v, nu, Rc)).replace(".", ","));
}
